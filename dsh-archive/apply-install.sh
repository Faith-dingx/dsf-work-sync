...
# 用法: sh apply-install.sh [dsh根目录, 默认 /srv/dsh]
...
# 3) 系统侧 init.d(占位符版本: DSH_HUB_SESSION_TOKEN 需按目标机中枢值填实)
[ -f "$HERE/system/init.d-dsh" ] && { [ -e /etc/init.d/dsh ] && cp -a /etc/init.d/dsh "/etc/init.d/dsh.pre-archive-restore"; cp "$HERE/system/init.d-dsh" /etc/init.d/dsh; }
[ -f "$HERE/system/init.d-config-ui" ] && { [ -e /etc/init.d/config-ui ] && cp -a /etc/init.d/config-ui "/etc/init.d/config-ui.pre-archive-restore"; cp "$HERE/system/init.d-config-ui" /etc/init.d/config-ui; }
echo "WARN: init.d 内 DSH_HUB_SESSION_TOKEN 为占位符 — 按目标机中枢 hubv2 data/hub-token 填实再启动"
grep -q "HUB_TOKEN — 从中枢" /etc/init.d/dsh 2>/dev/null && echo "      (当前 /etc/init.d/dsh 仍是占位符, 需人工填)" || echo "      (init.d token 已填实或旧版在位)"
